...
export KC_TOKEN=`curl -k --request POST --url http://localhost:8080/realms/master/protocol/openid-connect/token \
...
curl -k --silent --show-error -L -X POST "http://localhost:8080/admin/realms" \
--header "Content-Type: application/json" \
--header "Authorization: Bearer ""$KC_TOKEN" \
--data '{"realm": "geostudio", "enabled": true}'

curl -k --silent --show-error -L -X POST "http://localhost:8080/admin/realms/geostudio/clients" \
--header "Content-Type: application/json" \
--header "Authorization: Bearer ""$KC_TOKEN" \
...
client_uuid=`curl -k -X GET "http://localhost:8080/admin/realms/geostudio/clients?clientId=geostudio-client" \
--header "Content-Type: application/json" \
--header "Authorization: Bearer ""$KC_TOKEN" | jq -r '.[0].id' | tr -d '"'`

echo "Client UUID: $client_uuid"

export client_secret=`curl -k -X GET "http://localhost:8080/admin/realms/geostudio/clients/$client_uuid/client-secret" \
--header "Content-Type: application/json" \
...
curl -k --show-error -L -X POST "http://localhost:8080/admin/realms/geostudio/users" \
--header "Content-Type: application/json" \
--header "Authorization: Bearer ""$KC_TOKEN" \
--data '{"username": "testuser", "email": "test@example.com", "enabled": true, "firstName": "Test", "lastName": "User", "emailVerified": true}'

user_id=`curl -k -X GET "http://localhost:8080/admin/realms/geostudio/users?username=testuser" \
--header "Content-Type: application/json" \
--header "Authorization: Bearer ""$KC_TOKEN" | jq '.[0].id' | tr -d '"'`

echo "User ID: $user_id"

curl -k --silent --show-error -L -X PUT "http://localhost:8080/admin/realms/geostudio/users/$user_id/reset-password" \
--header "Content-Type: application/json" \
--header "Authorization: Bearer ""$KC_TOKEN" \
