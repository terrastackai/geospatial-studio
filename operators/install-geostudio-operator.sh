...
This script installs the GeoStudio OPERATOR (controller) only.
To deploy a GEOStudio application instance, use ./deploy-geostudio-lima.sh
...
Options:
  ...
  --apply-manifest     Also apply default GEOStudio CR (not recommended - use deploy-geostudio-lima.sh instead)
...
if [ "$APPLY_MANIFEST" = "false" ]; then
  ...
  echo ""
  echo "This will:"
  echo "  - Load secrets from workspace/<env>/env/.env"
  echo "  - Generate a GEOStudio CR from template"
  echo "  - Deploy the application to your cluster"
  echo ""
else
...
if [ "$DEPLOYMENT_MODE" = "local" ]; then
  ...
  echo "  ./build-studio-operators.sh"
